...
sub process {
    ...
    if ($self->{new}) {
        ...
        # This will use 1433 as server
        if ($self->{tcp}->{src_port} == 1433) {
        ...
        $self->{sref}->{"$client:$client_port:$server:$server_port"} = thicknet::session::MSSQL->new();
	...
	unless ($self->{session}->{sled}) {
	    if ($self->{dir}) {
		    # This will wait for a packet with 0x01 at the front to denote TDS
		    if (ord (substr($self->{tcp}->{data}, 0, 1)) == 0x01) {
		    # Uncomment to use any data packet, but many connections die too
		    # quickly to use. Need to start keeping those sessions open.
		    #if (length($self->{tcp}->{data}) > 0) {
